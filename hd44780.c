...
LCD_E_DIR 	|= LCD_E;   //
LCD_RS_DIR 	|= LCD_RS;  //
_delay_ms(15); // oczekiwanie na ustalibizowanie si̻ napiecia zasilajacego
...
void timer1_init(void)
{
	...
	TCCR1A = _BV(WGM11) |_BV(COM1A1)
			| _BV(COM1B1);	//Fast PWM mode, output at OCR1A and OCR1B
...
void ADC_init(void)
{
	 ADCSRA = 	(1<<ADEN) 		//ADC enable
	            |(1<<5) 		//Free run mode
	            |(1<<ADIE) 		//Enable interrupt reporting
	            |(1<<ADSC) 		//Start conversion
	            |(1<<ADPS0)   	//Set prescaler to 128
	            |(1<<ADPS1)
	            |(1<<ADPS2);
	 ADMUX  =    (1<<REFS0)  ; 	//VCC as reference voltage
}

void LED_init(void) //Initial sequence of LED
{
	PORTC |= (1<<LED1);
	PORTB |= (1<<LED2);
	PORTD |= (1<<LED3);
